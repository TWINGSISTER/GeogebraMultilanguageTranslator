...
//-----------------------------------------------------------------------
function translateWithWatson(text,sl,tl){ 	
	debugger;
...
function strTrans(text, sl, tl) {
	...
	var returnString= translateWithWatson(text,sl,tl);
...
function dictionary() {
	return sSet(RT_globlod("dictionary"));
}
...
function logmessage(message) {
	...
	globsto("logline"+i.toString(),base64EncodeUnicode(message));
...
function endLogging(win) {
	...
	for (let i = 1; i < fin; i++){
		...
		if (globExists(linetext)){
			var message=base64DecodeUnicode(RT_globlod(linetext))
